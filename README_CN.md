...
**输入方式：**
- **URL 模式** — 传入 `https://...` URL。默认 CLI 会先下载并本地转码后上传；若系统未安装 ffmpeg 或显式关闭转码，则直接将 URL 提交给 API。
- **文件模式** — 传入本地文件路径，CLI 读取、转码后以 base64 上传。

**本地转码管线（默认开启）：**

当 `PATH` 中存在 ffmpeg 时，CLI 会先把音频转为单声道 16kHz 32kbps MP3，并应用人声增强滤镜链（afftdn 降噪 → 80Hz 高通 → 两段 EQ → dynaudnorm 响度归一化）。上传体积通常缩小 10-20 倍，并在嘈杂录音上显著提升 ASR 效果。上传成功后临时文件会立即清理。

- 未检测到 ffmpeg 时，CLI 仅打印一次警告，随后按原样提交文件或 URL。
- 如果某些源文件与滤镜不兼容，会自动回退为无滤镜的普通转码（格式仍为目标 MP3）。
- 上传体积上限 **150 MB**：本地文件超限直接报错；URL 源转码后仍超限则回退为直接提交 URL。
- 加 `--no-transcode` 可跳过全部转码，按原文件上传。

**上层服务限制**（上传前本地校验）：

| 限制项 | 数值 | 说明 |
|--------|------|------|
| 最小文件大小 | 1 KB | 低于此值视为无效音频文件 |
| 最大文件大小 | 2 GB | 上层服务硬上限 |
| 最大音频时长 | 5 小时 | 有 ffprobe 时本地探测；否则由服务端拦截 |
