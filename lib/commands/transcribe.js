...
import { createClient, printResult, formatStatus, formatApiError } from '../utils.js';
import {
  MAX_UPLOAD_BYTES,
  MIN_FILE_BYTES,
  MAX_FILE_BYTES,
...
export function registerTranscribe(program) {
  program
    ...
    .option('--optimize', t('optOptimize'))
    .option('--standard', t('optStandard'))
    .option('--optimize-zh', t('optOptimizeZh'))
    .option('--timestamp-level <level>', t('optTimestampLevel'))
    .option('--beam-size <n>', t('optBeamSize'), parseInt)
    .option('--best-of <n>', t('optBestOf'), parseInt)
    ...
    .option('--vad-speech-pad <ms>', t('optVadSpeechPad'), parseInt)
    ...
    .option('--diarize-min-segment <s>', t('optDiarizeMinSegment'), parseFloat)
    .option('--diarize-single-speaker-threshold <n>', t('optDiarizeSingleSpeaker'), parseFloat)
    ...
    .action(async (source, opts) => {
      ...
      try {
        ...
        // Transcription tier (tri-state). `--optimize-zh` is a deprecated alias
        // for `--optimize`. When neither tier flag is given we omit `optimize`
        // entirely, letting the server auto-select by detected language
        // (zh/en/es/fr/pt → optimize tier, others → standard).
        const wantOptimize = opts.optimize === true || opts.optimizeZh === true;
        if (opts.standard && wantOptimize) {
          throw new Error(t('optimizeConflict'));
        }
        if (opts.standard) options.optimize = false;
        else if (wantOptimize) options.optimize = true;
        if (opts.timestampLevel) options.timestamp_level = opts.timestampLevel;
        if (opts.language) options.asr_language = opts.language;
        if (opts.beamSize != null) options.asr_beam_size = opts.beamSize;
        if (opts.bestOf != null) options.asr_best_of = opts.bestOf;
        ...
        if (opts.vadSpeechPad != null) options.vad_speech_pad_ms = opts.vadSpeechPad;
        ...
        if (opts.diarizeMinSegment != null) options.diarize_min_segment_sec = opts.diarizeMinSegment;
        if (opts.diarizeSingleSpeakerThreshold != null) options.diarize_single_speaker_threshold = opts.diarizeSingleSpeakerThreshold;
