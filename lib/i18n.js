const messages = {
  en: {
    ...
    optimizeConflict: '--optimize and --standard cannot be used together.',
    rateLimitedRetry: 'Rate limited, retrying in {sec}s (attempt {attempt})...',
    hintInsufficientBalance: 'Hint: available balance too low (funds may be held by pending tasks). Recharge or wait for tasks to finish — run `auralwise account`.',
    hintConcurrencyLimit: 'Hint: too many active tasks. Wait for some to finish, use --batch, or check `auralwise account` (available_concurrency).',
    hintRateLimited: 'Hint: request rate too high. Slow down or lower --tps; see your tps_limit via `auralwise account`.',
    // Global options
    optApiKey: 'API key (or set AURALWISE_API_KEY env)',
    optBaseUrl: 'API base URL',
    optLocale: 'UI language: en or zh (default: en)',
    optTps: 'Client-side request rate cap in req/s (match your account tps_limit; 0 = off)',
    optBurst: 'Client-side burst capacity for --tps pacing (default: 5)',
    optMaxRetries: 'Max auto-retries on 429 rate-limit before giving up (default: 5)',
    ...
    optOptimize: 'Optimize tier: faster & cheaper, segment-level timestamps (zh/en/es/fr/pt; other languages fall back to standard). Default: auto by language',
    optStandard: 'Standard tier: highest quality, word-level timestamps, all languages',
    optOptimizeZh: 'Deprecated alias for --optimize (applies to all languages now)',
    optTimestampLevel: 'Timestamp granularity: word or segment (optimize tier only; experimental)',
    optBeamSize: 'Beam search width',
    optBestOf: 'Best-of sampling count, 1-20 (only when --temperature > 0)',
    ...
    optVadSpeechPad: 'Speech segment padding in ms',
    ...
    optDiarizeMinSegment: 'Min segment duration (sec) for embedding extraction',
    optDiarizeSingleSpeaker: 'Single-speaker detection threshold (silhouette score)',
    ...
    acctBalance: 'Balance',
    acctHeld: 'Held',
    acctResourcePack: 'Resource pack',
    acctAvailable: 'Available',
    acctTier: 'Tier',
    acctConcurrency: 'Concurrency',
    acctConcurrencyValue: '{active} active / {limit} limit ({available} available)',
    acctWaitingBatch: 'Batch in progress',
    acctRateLimit: 'Rate limit',
    acctRateLimitValue: '{tps} req/s (burst {burst})',
    acctRateLimiting: 'Rate limiting',
    acctExempt: 'exempt (admin)',
    ...
    descAccount: 'Show account balance, limits, and usage',
  ...
  zh: {
    ...
    optimizeConflict: '--optimize 与 --standard 不能同时使用。',
    rateLimitedRetry: '触发请求限速，{sec} 秒后重试（第 {attempt} 次）...',
    hintInsufficientBalance: '提示：可用余额不足（部分可能被处理中任务冻结）。请充值或等待任务完成 —— 可运行 `auralwise account` 查看。',
    hintConcurrencyLimit: '提示：活跃任务数已达并发上限。请等待部分任务完成、改用 --batch，或运行 `auralwise account` 查看 available_concurrency。',
    hintRateLimited: '提示：请求速率过高。请放慢或调低 --tps；可运行 `auralwise account` 查看 tps_limit。',
    // Global options
    optApiKey: 'API 密钥（或设置 AURALWISE_API_KEY 环境变量）',
    optBaseUrl: 'API 基础 URL',
    optLocale: '界面语言：en 或 zh（默认 en）',
    optTps: '客户端请求速率上限（次/秒，建议匹配账户 tps_limit；0=关闭）',
    optBurst: '客户端 --tps 节流的突发容量（默认 5）',
    optMaxRetries: '触发 429 限速时的最大自动重试次数（默认 5）',
    ...
    optOptimize: '优化档：更快更省，段级时间戳（中/英/西/法/葡；其他语言自动落标准档）。默认按语言自动选档',
    optStandard: '标准档：最高质量，词/字级时间戳，全语言',
    optOptimizeZh: '已废弃：等价于 --optimize（现对全语言生效）',
    optTimestampLevel: '时间戳粒度：word 或 segment（仅优化档生效；实验性）',
    optBeamSize: 'Beam Search 宽度',
    optBestOf: 'Best-of 采样数 1-20（仅 --temperature > 0 时生效）',
    optTemperature: '解码温度',
    optHotwords: '热词（逗号分隔）',
    ...
    optVadSpeechPad: '语音段前后填充时长（毫秒）',
    optNoSpeechThreshold: '无语音概率阈值',
    optNumSpeakers: '固定说话人数',
    optMinSpeakers: '自动检测最少说话人数',
    optMaxSpeakers: '自动检测最多说话人数',
    optDiarizeMinSegment: '提取声纹所需最短语音段时长（秒）',
    optDiarizeSingleSpeaker: '单说话人判定阈值（Silhouette Score）',
    ...
    acctBalance: '余额',
    acctHeld: '冻结（处理中预占）',
    acctResourcePack: '资源包等值额度',
    acctAvailable: '可用额度',
    acctTier: '账户等级',
    acctConcurrency: '并发任务',
    acctConcurrencyValue: '{active} 活跃 / {limit} 上限（还可提交 {available}）',
    acctWaitingBatch: '批量处理中',
    acctRateLimit: '请求速率上限',
    acctRateLimitValue: '{tps} 次/秒（突发 {burst}）',
    acctRateLimiting: '限流',
    acctExempt: '豁免（管理员）',
